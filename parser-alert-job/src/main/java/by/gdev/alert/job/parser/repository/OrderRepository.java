...
public interface OrderRepository extends CrudRepository<Order, Long> {
    ...
    Set<Order> findAllBySourceOneEagerTechnologies(Long source, Long category, Long subCategory, Date date);

    @Query("select o from parser_order o left join fetch o.technologies left join fetch o.sourceSite s "
	    + "where s.source = :source and s.category = :category and s.subCategory is null and o.dateTime >= :date")
    Set<Order> findAllBySourceSubCategoryIsNullOneEagerTechnologies(Long source, Long category, Date date);
}