...
import {
  AutoreplySitesSupportingApi,
  UserCredentialsApi,
  type UserCredentialRequest,
  type UserSiteCredentialShortResponse,
} from '@/apis/coreApi';
...
export const AccountDialog = ({ isOpen, formState, initialFields, close }: Props) => {
  ...
  const submit = ({ login, name, password, siteId }: FormValues) => {
    createOrUpdateAccount({ login, name, password, siteId });
  };
        ...
        <DialogContent className="account-dialog__content">
          ...
          {!isLoading && (
            <FormControl variant="standard" size="small" required error={Boolean(errors.siteId)}>
              ...
              <Select value={siteId} label="Сайт" {...register('siteId', { required: true })} disabled={formState === 'editing'}>
